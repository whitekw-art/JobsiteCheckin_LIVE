...
- [ ] **Feature gating** — No gating exists. All tiers get identical functionality. Build BEFORE welcome/onboarding flow. Use `hasFeature(org.planTier, org.planVersion, 'feature_name')` pattern from design doc. See `docs/plans/2026-03-11-registration-gating-design.md`.
- [x] **Auto sign-in after registration** — `register/page.tsx` now calls `signIn('credentials', { email, password, redirect: false })` immediately after account creation (password still in memory). On success → `/pricing` (authenticated). On failure → `/auth/signin?email=...`. Applies to all users regardless of plan.
- [x] **Free plan path** — `POST /api/auth/select-free-plan` (session-protected): sets `planTier='free'`, `planVersion=1` on org, returns `{ redirect: '/dashboard' }`. `app/pricing/FreePlanButton.tsx` calls this API; user arrives at `/pricing` already authenticated so the call succeeds cleanly. No manual sign-in screen at any point in the free or paid flow.
- [x] **Plan versioning / grandfathering** — `planVersion Int @default(1)` on Organization schema. `lib/planVersions.ts` created with `CURRENT_PLAN_VERSION`, `PLAN_FEATURES` map, and `hasFeature()` helper. Webhook writes `planVersion` on checkout. NEVER edit existing version rows — only add new ones.
...
- [x] **Landing page CTA swap** — `app/page.tsx` reads `REGISTRATION_OPEN` env var (server-side) and passes as `registrationOpen` prop to `LandingPage`. When `true`: nav shows "Sign In / Register" and removes waitlist button; hero, all 8 pricing table buttons, and CTA band say "Get Started" and route to `/auth/register`; waitlist modal not rendered. When `false`: all existing waitlist behavior unchanged.
- [ ] **Landing page copy cleanup for launch** — When `REGISTRATION_OPEN=true`, copy referencing waitlist/founding-member language still shows: urgency bar ("Only 20 founding member spots"), hero badge ("EARLY ACCESS — ONLY 20 SPOTS"), trust row ("lock in 50% off forever"), CTA band subtext ("Free to join. Founding members get 50% off for life."), pricing "founding member rate" labels. These need conditional variants. Do not implement until user is ready to flip the switch.
